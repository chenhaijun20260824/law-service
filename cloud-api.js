/**
 * cloud-api.js — 法律服务 云端适配器（GitHub Repo as DB）
 * 实现 window.RoseSyncAdapter 接口（pull / push / status）。
 *
 * 配置来源（优先级）：
 *   1) localStorage 键 'law_cloud_cfg'
...
(function () {
  'use strict';
  var LS_KEY = 'law_cloud_cfg';

  // 仓库/分支/路径固定不变，每台设备只需填 Token
  var DEFAULTS = { owner: 'chenhaijun20260824', repo: 'law-service-data', branch: 'main', path: 'data' };
  function norm(c) {
    c = c || {};
    return {
      token: c.token,
  ...
  if (cfg) {
    ...
    try { localStorage.setItem('law_server_online', '1'); } catch (e) {}
  } else {
    window.百合_openCloudSetup = function () { location.href = 'cloud-setup.html'; };
  }

  console.log('[法律服务] cloud-api.js (GitHub Repo) loaded — ' + (cfg ? cfg.owner + '/' + cfg.repo + ' / ' + cfg.path : 'not configured → local mode'));
})();